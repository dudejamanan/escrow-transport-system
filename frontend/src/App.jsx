...
// backend API base
const API_BASE = "http://localhost:3000/api/escrow";
  ...
  try {
    ...
    if (!res.ok) {
      const message =
        ...
        (typeof payload === "string" ? payload : "Unexpected backend error");
    ...
    <div className="min-h-screen flex">
      ...
      <div className="flex-1 flex items-center justify-center px-6 py-16" style={{ background: "#f8fafc" }}>
        <div className="w-full max-w-[400px]">
          ...
          <p className="text-center text-xs text-slate-400 mt-8">© 2026 SwiftEscrow · Secured &amp; encrypted</p>
        </div>
      </div>
    </div>
  );
...
const EscrowPage = ({ dark, showToast }) => {
  ...
  const loadEscrows = useCallback(async () => {
    setLoading(true);
    try {
      const data = await apiFetch("/escrows", { method: "GET" });
      if (!Array.isArray(data)) {
        throw new Error("Unexpected /escrows response");
      }
      setEscrows(data);
    } catch (err) {
      console.error(err);
      showToast(err.message || "Failed to load escrows", "error");
    } finally {
      setLoading(false);
    }
  }, [showToast]);
  ...
  const totals = escrows.reduce(
    (acc, row) => {
      ...
      acc.totalAmount += amt;
      if (row.status === "LOCKED") acc.locked += amt;
      if (row.status === "RELEASED") acc.released += amt;
      if (row.status === "REFUNDED") acc.refunded += amt;
      return acc;
    },
    { totalAmount: 0, locked: 0, released: 0, refunded: 0 }
  );
  ...
  const handleCreateEscrow = async (e) => {
    e.preventDefault();
    if (!createForm.buyer_wallet || !createForm.seller_wallet || !createForm.amount) {
      showToast("Fill buyer, seller, and amount", "error");
      return;
    }
    setActionId("create");
    try {
      const payload = await apiFetch("/createEscrow", {
        method: "POST",
        body: JSON.stringify({
          buyer_wallet: createForm.buyer_wallet,
          seller_wallet: createForm.seller_wallet,
          amount: Number(createForm.amount)
        })
      });
      showToast("Escrow created", "success");
      if (payload && payload.escrow) {
        setEscrows((prev) => [payload.escrow, ...prev]);
      } else {
        await loadEscrows();
      }
      setCreateForm({ buyer_wallet: "", seller_wallet: "", amount: "" });
    } catch (err) {
      console.error(err);
      showToast(err.message || "Failed to create escrow", "error");
    } finally {
      setActionId(null);
    }
  };

  const withRowAction = async (orderId, fn, successMsg) => {
    if (!orderId) return;
    ...
    try {
      ...
      if (result && result.escrow) {
        const updated = result.escrow;
        setEscrows((prev) =>
          prev.map((row) =>
            String(row.order_id) === String(orderId) ? updated : row
          )
        );
      } else {
        await loadEscrows();
  ...
  const fetchStatus = async (orderId) => {
    await withRowAction(
      orderId,
      (id) => apiFetch(`/status/${encodeURIComponent(id)}`, { method: "GET" }),
      undefined
    ).then((status) => {
      if (status && status.orderId != null) {
        setStatusByOrder((prev) => ({
          ...prev,
          [String(status.orderId)]: status
        }));
        showToast(
          `Escrow ${status.orderId} is ${status.status}`,
          "success"
        );
      }
    });
  ...
  const submitDelivery = (orderId) =>
    withRowAction(
      orderId,
      (id) => apiFetch(`/submitDelivery/${encodeURIComponent(id)}`, { method: "POST" }),
      `Delivery submitted for order ${orderId}`
    );

  const confirmDelivery = (orderId) =>
    withRowAction(
      orderId,
      (id) => apiFetch(`/confirmDelivery/${encodeURIComponent(id)}`, { method: "POST" }),
      `Delivery confirmed for order ${orderId}`
    );

  const refundEscrow = (orderId) =>
    withRowAction(
      orderId,
      (id) => apiFetch(`/refund/${encodeURIComponent(id)}`, { method: "POST", body: JSON.stringify({}) }),
  ...
  return (
    <div className="page-fade space-y-5">
      <Banner
        ...
        sub="Live on-chain escrow tracking"
        img="https://images.unsplash.com/photo-1611974789855-9c2a0a7236a3?w=1400&h=300&fit=crop"
        accent="linear-gradient(90deg,rgba(9,14,33,.92),rgba(6,182,212,.5))"
      />

      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { l: "Total Escrow", v: `Ξ${formatEth(totals.totalAmount)}`, I: Lock, bg: "bg-sky-600" },
          { l: "Locked", v: `Ξ${formatEth(totals.locked)}`, I: AlertTriangle, bg: "bg-amber-500" },
          { l: "Released", v: `Ξ${formatEth(totals.released)}`, I: CheckCircle, bg: "bg-emerald-600" },
          { l: "Refunded", v: `Ξ${formatEth(totals.refunded)}`, I: ArrowDownRight, bg: "bg-violet-600" }
        ].map((s, i) => (
          <div key={i} className={`ch ${card} rounded-2xl p-5`}>
            <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${s.bg} mb-3 shadow-sm`}>
              <s.I size={16} className="text-white" />
            </div>
            <p className={`syne text-xl font-bold ${c}`}>{s.v}</p>
            <p className={`text-xs mt-0.5 ${cs}`}>{s.l}</p>
          </div>
        ))}
      </div>

      <div className={`${card} rounded-2xl p-4 space-y-3`}>
        <p className={`text-sm font-bold ${c}`}>Create test escrow (hits POST /createEscrow)</p>
        <form
          className="grid grid-cols-1 md:grid-cols-4 gap-3 items-end"
          onSubmit={handleCreateEscrow}
        >
          <div>
            <label className={`block text-xs mb-1 ${cs}`}>Buyer wallet</label>
            <input
              className="w-full px-3 py-2 rounded-xl border text-xs bg-slate-900/60 border-slate-700 text-slate-200"
              value={createForm.buyer_wallet}
              onChange={(e) =>
                setCreateForm((f) => ({ ...f, buyer_wallet: e.target.value }))
              }
              placeholder="0xBuyer..."
            />
          </div>
          <div>
            <label className={`block text-xs mb-1 ${cs}`}>Seller wallet</label>
            <input
              className="w-full px-3 py-2 rounded-xl border text-xs bg-slate-900/60 border-slate-700 text-slate-200"
              value={createForm.seller_wallet}
              onChange={(e) =>
                setCreateForm((f) => ({ ...f, seller_wallet: e.target.value }))
              }
              placeholder="0xSeller..."
            />
          </div>
          <div>
            <label className={`block text-xs mb-1 ${cs}`}>Amount (ETH)</label>
            <input
              type="number"
              step="0.0001"
              min="0"
          ...
          <button
            ...
            className="px-4 py-2.5 rounded-2xl text-white text-xs font-bold bp flex items-center justify-center gap-2"
          >
            {actionId === "create" ? "Creating…" : "Create escrow"}
          </button>
        </form>
      </div>
          ...
          <tbody className="divide-y" style={{ borderColor: dark ? "#1e293b" : "#f8fafc" }}>
            {escrows.length === 0 && !loading && (
              <tr>
                <td
                  colSpan={6}
                  className={`text-center py-10 text-sm ${cs}`}
                >
                  No escrow rows yet.
                </td>
              </tr>
            ...
            {escrows.map((row) => {
              ...
              const statusInfo = statusByOrder[String(orderId)];
              return (
                <tr
                  key={String(orderId)}
                  className={`transition-colors ${dark ? "dm-row" : "hover:bg-slate-50/70"}`}
                >
                  <td className="px-4 py-3.5 text-xs font-mono font-bold text-indigo-500">
                    {orderId}
                  </td>
                  <td className={`px-4 py-3.5 text-sm font-bold ${c}`}>
                    Ξ{formatEth(row.amount)}
                  </td>
                  <td className="px-4 py-3.5">
                    <SB status={status?.toLowerCase()} />
                  </td>
                  <td className={`px-4 py-3.5 text-xs font-mono ${cs}`}>
                    {row.contract_address || "—"}
                  </td>
                  <td className={`px-4 py-3.5 text-xs ${cs}`}>
                  ...
                  <td className="px-4 py-3.5">
                    <div className="flex flex-wrap gap-1.5">
                      <button
                        ...
                        disabled={actionId === String(orderId)}
                        className="px-2.5 py-1.5 rounded-xl text-[11px] font-bold bg-slate-800 text-slate-200 hover:bg-slate-700 transition-colors"
                      >
                        Status
                      </button>
                      <button
                        ...
                        disabled={actionId === String(orderId)}
                        className="px-2.5 py-1.5 rounded-xl text-[11px] font-bold bg-sky-600 text-white hover:bg-sky-500 transition-colors"
                      >
                        Submit
                      </button>
                      <button
                        ...
                        disabled={actionId === String(orderId)}
                        className="px-2.5 py-1.5 rounded-xl text-[11px] font-bold bg-emerald-600 text-white hover:bg-emerald-500 transition-colors"
                      >
                        Confirm
                      </button>
                      <button
                        type="button"
                        onClick={() => refundEscrow(orderId)}
                        disabled={actionId === String(orderId)}
                        className="px-2.5 py-1.5 rounded-xl text-[11px] font-bold bg-red-600 text-white hover:bg-red-500 transition-colors"
                      >
                        Refund
                      </button>
                    </div>
                    {statusInfo && (
                      <p className={`mt-1 text-[11px] ${cs}`}>
                        Chain status: {statusInfo.status} · Ξ
                        {formatEth(statusInfo.amount)} ·{" "}
